...
;Rev 2a 06/08/13 Initial version
;Rev 2b 20/08/13 Fix bugs
;Rev 2c 24/08/13 Fix more bugs
;Rev 2d 28/08/13 Clear PORT C when all events are deleted
;Rev 2e 31/08/13 Flags parameter corrected to return PF_COMBI

;Rev 102h 02/11/13 Start coding to support short event generation
;Rev 102j 03/11/13 Various bug fixes
...
; define for development testing using MPLAB_SIM
;#define TESTING
...
RQDEVID equ 0x4E  ; request device Id
DEVID equ 0x8E  ; Device Id reply

EN_NUM  equ .128  ;number of allowed events

EV_NUM  equ .17   ;number of allowed EVs per event

COND_ID equ 28    ; id for CANLED64
...
; define RAM storage
  ...
  MtrxChk   ;check word for matrix condition chamge
  MtrxRoll  ;rolling bit to set MtrxChk
  ...
  Matrix    ;8 bytes for condition array
  Matrix1
  Matrix2
  Matrix3
  Matrix4
...
sendev
    ...
    sublw 9
    movwf Tx1d4
    ; now check for a short event
    movwf Temp
    decf  Temp  ;zero based index to shortevs in EEPROM
    rlncf Temp
    movlw LOW shortevs
    addwf Temp,w
    movwf EEADR ;read short event data
    call  eeread
    movwf EVtemp
    incf  EEADR
    call  eeread
    movwf EVtemp1
    addwf EVtemp,w
    bz    longev  ; j if short event not set
    movff EVtemp, Tx1d3
    movff EVtemp1, Tx1d4
    bsf   Tx1d0,3   ; change opcode to short version
longev
    movlw 5
    movwf Dlc
    call  sendTX
...
;***************************************************************************
;   main setup routine - copied from CANACC8
;*************************************************************************
...
;   movwf TMR1L     ;set for 20ms tick
    ...
    clrf  PIE2
;   movlw B'00000001'
    clrf  PIE1      ;enable interrupt for timer 1
...
seten_f 
    call  cpyNvs      ; copy NVs to Matrix, set SOD condition
    movlw B'11000000'
...
lrn2
  lfsr  FSR0,led00    ; point at condition data in lower block
  lfsr  FSR2,Matrix
  movlw 0x20      ; check if ev is in lower or upper block
  andwf evaddrl,w
  btfss STATUS, Z   ; j if lower block
  lfsr  FSR0, led10   ; point at condition data in upper block
  movlw 0x08      ; check if updating POL bit or shortev flag
  cpfslt  EVidx   
  bra   lrn2a     ; j if yes
  movf  EVidx,w
  movff PLUSW0, EVtemp1 ; new data
  movf  PLUSW2,w    ; get matrix data
  comf  EVtemp1
  movf  PLUSW2,w    ; get current matrix data
  andwf EVtemp1     ; clear conditions in new data
  iorwf EVdata,w    ; or into current data
  movwf EVtemp1
  movf  EVidx,w
  movff EVtemp1,PLUSW2  ; write back to Matrix
  movff PLUSW0, EVtemp1 ; current event data
  comf  EVtemp1
  movlw LOW condctrl  ; now update NVs
  addwf EVidx,w
  movwf EEADR
  call  eeread      ; get current value
  andwf EVtemp1,w   ; remove current data
  iorwf EVdata,w    ; add in new data
  call  eewrite
...
endlrn2
  ...
  movlw .16       ; zero based EV index
  cpfseq  EVidx     ;j if writing short event flag
  retlw 0
  
  movf  EVdata,w
  bz    endlrn3     ;j if not a short event
  decf  EVdata      ; make shortev index zero based
  rlncf EVdata
  movlw LOW shortevs
  addwf EVdata,w
  movwf EEADR
...
unl3
  ...
  movf  EVdata,w    ; not a short event
  bz    unl3a
  decf  EVdata      ; zero based index
  rlncf EVdata
  movlw LOW shortevs
  addwf EVdata,w
  movwf EEADR     ; clear short event values
  clrf  WREG
  call  eewrite
  incf  EEADR
  clrf  WREG
...
remove      ; remove all conditions for event from Matrix and EEPROM
  movlw 8
  movwf Count3
  movlw LOW condctrl
  movwf EEADR
...
initevdata    ; clear all event info
  clrf  PORTC     ; turn off any outputs
  ...
  call  nv_rest     ;clear conditions and matrix
  movlw .128
  movwf Count 
  movlw LOW hashtab
  movwf EEADR
...
nexthn            ; clear hash table count
  ...
  movlw .64
  movwf Count   ; loop 64 times for 128 events
nxtev
  movlw .32
  addwf evaddrl,w
  movwf Temp
...
clrev   ; erase all of the event data area in flash
  movlw EN_NUM/2
  movwf Count   ; do EN_NUM/2 * 64 blocks, 2 events per block
  movlw 0
  movwf TBLPTRL
  movlw high evdata
  movwf TBLPTRH
...
wrfb
  ...
  movwf CountFb1
nxt32
  movlw .31     ; only write 31 bytes in loop
  movwf CountFb0
nxtfb
  movf  POSTINC0, W
  movwf TABLAT
  ...
  movf  POSTINC0,w
  movwf TABLAT
  tblwt*          ; write 32nd data byte, leave TBLPTRL alone
  call  wrflsh
  decfsz  CountFb1
  bra   dofb32
  return
dofb32
  incf  TBLPTRL     ; step into next 32 byte block
  lfsr  FSR0, evt10
  bra   nxt32
...
;
clrmtrx
    lfsr  FSR2, Matrix    ;clear condition matrix
...
;************************************************************************
; routine to clear NVs and Matrix
...
; clear shortevs, 16 bytes 2 per condition
    movlw .16
    movwf Count
    movlw LOW shortevs
    movwf EEADR
...
; returns Device Id from chip in PRODH and PRODL
idread
    movlw 0x3F
    movwf TBLPTRU
    movlw 0xFF
    ...
    movwf TBLPTRL
    tblrd*+
    movff TABLAT, PRODL ;contains ls 3 bits of Devid in bits7-5
    tblrd*
    movff TABLAT, PRODH ;contains rest of Devid in bits 7-0
    rlncf PRODL     ;get ms 3 bits to ls end
    rlncf PRODL
    rlncf PRODL
    movlw 0x07  
    andwf PRODL     ;extract ls 3 bits
    rlncf PRODH     ;rotate by 3 bits to put...
    rlncf PRODH     ;...ls 5 bits at ms end
    rlncf PRODH
    movlw 0xf8      
    andwf PRODH,w     ; extract ls 5 bits...
    iorwf PRODL     ; merge with ls 3 bits
    movlw 0x07      
    andwf PRODH     ; extract ms 3 bits of devid
...
ENCount de  0,EN_NUM  ; no of events in event data/free space
...
; condition control, 8 conditions, 8 events (max) per condition
; a condition is true when all bits are zero, any 1 bit blocks the condition
; table holds a bit per event condition, copied to matrix at sod
condctrl  de 0,0,0,0,0,0,0,0

; short evs, holds the device number (2 bytes) for each condition
; if zero, then the condition generates a normal event
